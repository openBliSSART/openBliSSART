...
bool SpectralAnalysisTest::performTest(int nSamples, int windowSize)
{
    ...
    std::unique_ptr<Matrix> pAmplitudeMatrix(spectrogram.first);
    std::unique_ptr<Matrix> pPhaseMatrix(spectrogram.second);
    ...
    std::unique_ptr<AudioData> pAudioData2(AudioData::fromSpectrogram(*pAmplitudeMatrix, *pPhaseMatrix,
